...
import { ILegendaryGroup, IMonsterFile, IMonsterMetaFile, IStoredMonster, IStoredMonsterFeat, isTypeWithEntries } from "../../models";
...
export class MonsterImporter implements IImporter {
	...
	public async getData() {
		...
		const metaFile: IMonsterMetaFile = await this.fileGetter.getResource(["bestiary", "meta.json"].join("/"));

		console.log("Converting monster data...");
		const legendaryGroupMap = metaFile.legendaryGroup.reduce((map, legendaryGroup) =>
